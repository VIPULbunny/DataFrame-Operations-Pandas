...
# Select and display the column 'Grapes' as a DataFrame
print(df[['Grapes']])  

# Add a new column 'Banana', calculated by adding 150 to 'Grapes'
df["Banana"] = df["Grapes"] + 150  

# Add a column 'pear' calculated by adding 15 to 'Grapes'
df["pear"] = df["Grapes"] + 15  

# Add a column 'strawberry' by adding 50 to 'pear'
df["straberry"] = df["pear"] + 50  

# Add a column 'oranges' by adding 10 to 'Grapes'
df["oranges"] = df["Grapes"] + 10  

# Add a column 'kiwi' by adding 120 to 'strawberry'
df["kiwi"] = df["straberry"] + 120  

# Add a column 'mangoes' by adding 100 to 'kiwi'
df["mangoes"] = df["kiwi"] + 100  

# Example of row selection using loc and iloc (commented for optional use):
...
# Display statistical exploration of the DataFrame (commented for optional use):
